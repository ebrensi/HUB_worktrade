...
@author: Efrem
"""
# This script is gonna log into OurVolts and scrape volunteers' hours
# information
...
dd = (dir_list
      .str
      .extract('hours-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)')
      .dropna()
      .sort([0, 1, 2, 3, 4, 5], ascending=False))
...
for volunteer in volunteers:
    print("{} worked...".format(volunteer))
    mask = ((df['Volunteer'] == volunteer) &
            (df.Date > start_date) &
            (df.Date < end_date))
